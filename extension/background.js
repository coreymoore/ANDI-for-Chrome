...
// Uses the scripting API to inject jquery and ANDI into the active tab (ISOLATED world)
...
chrome.action.onClicked.addListener(async (tab) => {
  ...
  // Step 2: Always inject local jQuery
  // In the Isolated World, we cannot see the page's jQuery, so we must always inject ours.
  await chrome.scripting.executeScript({
    target: { tabId },
